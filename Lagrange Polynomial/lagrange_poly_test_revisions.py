class Lagrange_Poly:
    ...
    def __init__(self):
        ...
        self.pruned_x_seq = []
        self.pruned_y_seq = []
        self.secondary_seq_length = 0
        self.secondary_y_seq = []
        self.is_set = 0
        self.was_pruned = 0
    ...
    def set_polynomial_multi(self, seq_length, num_seq, x_seq, y_seq):
        ...
        for k in range(seq_length):
            ...
            self.secondary_y_seq.append("")
            self.secondary_seq_length += 1
            if (len(str(y_seq[k])) != 0):
                self.pruned_seq_length += 1
                self.pruned_y_seq.append(y_seq[k])
            else:
                self.unknown_indexes.append(k)
        if (self.pruned_seq_length != self.seq_length):
            self.was_pruned = 1
    def set_polynomial_range(self, eq_length, start, end, x_seq, y_seq):
        self.set_polynomial(end-start, x_seq[start:end], y_seq[start:end])

    def set_polynomial_multi_range(self, seq_length, start, end, num_seq, x_seq, y_seq):
        arr = []
        for i in range(num_seq):
            arr.append(x_seq[i][start:end])
        self.set_polynomial_multi(end - start, num_seq, arr, y_seq[start:end])
    ...
    def create_error_check_poly (self):
        if (self.is_set):
            poly = Lagrange_Poly()
            

            for a in range (len(self.unknown_indexes)):
                self.secondary_y_seq[self.unknown_indexes[a]] = self.y_seq[self.unknown_indexes[a]]
                print(self.unknown_indexes[a])

            poly.set_polynomial_multi(self.seq_length, self.num_seq, self.origonal_x_seq, self.secondary_y_seq)
            poly.set_accuracy(self.accuracy)
            ...
            print(str(poly.solve_for_x(0.427)))
            print(str(poly.solve_for_x(0.191)))
...
def main():
    ...
    print(str(poly.solve_for_x([0.148])))
    print(str(poly.solve_for_x([0.153])))
    print(str(poly.solve_for_x([0.5])))
    poly.fill_in_missing_values()
    poly.print_y_seq()
